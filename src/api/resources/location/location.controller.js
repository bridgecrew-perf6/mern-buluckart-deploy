...
export default {
    ...
    async index(req, res, next) {
        try {
            const { zipcode, name, status,latitude,longitude,hex,mapUrl } = req.body;
            db.location.findOne({ where: { name: name } })
                .then(data => {
                    if (data) {
                        return db.location.update({ 
                            zipcode: zipcode, name:name ,
                            status:parseInt(status)?'active':'inactive' }, { where: { id: data.id } ,
                            latitude:latitude,
                            longitude:longitude,
                            hex:hex,
                            mapUrl:mapUrl
    ...
    async areaCreate(req, res, next) {
        try {
              const { zipcode, name,locationId, status,latitude,longitude,hex,mapUrl } = req.body;
    ...
    async getAreaList(req, res, next) {
        try {
            db.area.findAll({
                ...
                include: [{ model: db.location, attributes: ["id", "name" ,"zipcode", "status","latitude","longitude","hex","mapUrl"] }]
            })
                .then(list => {
                    res.status(200).json({ 'success': true, data: list });
                })
